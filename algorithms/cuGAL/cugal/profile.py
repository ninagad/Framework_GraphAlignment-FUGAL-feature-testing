...
class SinkhornProfile:
    ...
    def average(profiles: list[Self]):
        ...
        iteration_count = int(
            sum([profile.iteration_count for profile in profiles]) / len(profiles))
...
class Profile:
    ...
    def from_dict(cls, dict: dict):
        dict['sinkhorn_profiles'] = [SinkhornProfile(
            **profile) for profile in dict['sinkhorn_profiles']]
        dict['phase_times'] = {
            Phase[phase]: time for phase, time in dict['phase_times'].items()}
        return cls(**dict)

    @staticmethod
    def average(profiles: list[Self]):
        phase_times = {phase: sum([p.phase_times[phase] for p in profiles]) / len(
            profiles) for phase in profiles[0].phase_times.keys()}
        time = sum([profile.time for profile in profiles]) / len(profiles)
        max_memory = max(
            [profile.max_memory for profile in profiles if profile.max_memory is not None], default=0)
        sinkhorn_count = len(profiles[0].sinkhorn_profiles)
        frank_wolfe_iterations = np.mean(
            [profile.frank_wolfe_iterations for profile in profiles])
...
def append_phases_to_csv(profile: Profile, path: str):
    ...
    writer = csv.writer(csvfile, delimiter=',',
                        quotechar='|', quoting=csv.QUOTE_MINIMAL)
    for row, phase in enumerate([f for f in phases]):
