...
def marginal_error_log(K: torch.Tensor, log_u: torch.Tensor, log_v: torch.Tensor, config: Config) -> float:
    if can_use_cuda(config):
    ...
    def solve_log(
        ...
        for iteration in range(config.sinkhorn_iterations):
            ...
            if iteration % config.sinkhorn_eval_freq == 0 and iteration != 0:
                errors.append(marginal_error_log(K, self.u, self.v, config))
